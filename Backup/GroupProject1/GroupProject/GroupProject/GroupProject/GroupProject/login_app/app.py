...
# -------------------------
# -------------------------
# Admin và User mặc định
# -------------------------
ADMIN_USER = {"email": "admin@gmail.com", "password": "123456", "role": "Admin"}
NORMAL_USER = {"email": "user@gmail.com", "password": "123456", "role": "User"}
...
def login():
    ...
    if request.method == "POST":
        ...
        if email == ADMIN_USER["email"] and password == ADMIN_USER["password"]:
            session["user"] = {"email": email, "role": "Admin", "name": "Administrator"}
        ...
        # --- User ---
        if email == NORMAL_USER["email"] and password == NORMAL_USER["password"]:
            session["user"] = {"email": email, "role": "User", "name": "User"}
            resp = make_response(redirect(url_for("success")))
            if remember:
                resp.set_cookie("email", email, max_age=60 * 60 * 24 * 7)
                resp.set_cookie("password", password, max_age=60 * 60 * 24 * 7)
        ...
        # --- Kiểm tra trong file JSON nếu có ---
        try:
...
def go_home():
    """Sau khi bấm nút 'Đóng' ở success.html → chuyển đúng trang home"""
...
def upload_dataset():
    ...
    if not file:
        return redirect(url_for("quanlidulieu"))

    # Lưu tạm vào thư mục data/
    ...
    return render_template("quanlidulieu.html", data=data)
...
def Uthongke():
    return render_template("Uthongke.html")
@app.route("/Uphanloaikhachhang")
def Uphanloaikhachhang():
    return render_template("Uphanloaikhachhang.html")
@app.route("/Uchienluocgoiytudong")
def Uchienluocgoiytudong():
    return render_template("Uchienluocgoiytudong.html")
@app.route("/Utoiuuhoachiendich")
def Utoiuuhoachiendich():
    return render_template("Utoiuuhoachiendich.html")
# Run
